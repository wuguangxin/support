...
public class GestureView extends View {
    ...
    /** 在设置手势时，第二次与第一次不一致 */
    public static final int ERROR_UNLIKE = 2;
    /** 解锁密码不正确 */
    public static final int ERROR_UNLOCK_FAILED = 3;
    /** 延时恢复组件时间(毫秒) */
    ...
    // ================== 配置变量 ==================================================================
    // 背景
    private int backgroundColorNormal = 0xfff2f2f2;        // 背景颜色 正常
    private int backgroundColorTouch = 0x573366ff;        // 背景颜色 触摸
    private int backgroundColorError = 0x66ff3366;        // 背景颜色 错误
    // 外圆
    private int outerCycleColorNormal = 0xfff2f2f2;        // 外圆颜色 正常
    private int outerCycleColorTouch = 0x573366ff;        // 外圆颜色 触摸
    private int outerCycleColorError = 0x66ff3366;        // 外圆颜色 错误
    // 内圆
    private int innerCycleColorNormal = 0x00000000;        // 内圆颜色 正常
    private int innerCycleColorTouch = 0xff3366ff;        // 内圆颜色 触摸
    private int innerCycleColorError = 0xffff3366;        // 内圆颜色 错误
    // 边框
    private float outerCycleStrokeSize = 10F;            // 外圆边框大小
    private float innerCycleStrokeSize = 10F;            // 内圆边框大小
    private float backgroundStrokeSize = 10F;            // 背景边框大小
    private float linkLineStrokeSize = 2.5F;            // 连接线条大小
    // 连接线
    private int linkLineColorNormal = 0x333366ff;        // 线条颜色 正常
    private int linkLineColorError = 0x33FF3366;        // 线条颜色 错误
    // 其他
     ...
     * 是否使用触觉反馈
     */
    private boolean touchShake;

    /**
     * 是否开启防误触功能。如果未true，则当按下并抬起时，计算链接的点数只有1点时，算是误触碰的，不算入绘制错误范围
     */
    private boolean avertUnawareTouch = false;
...
//		printInfo("初始化：measureWidth="+measureWidth);
//		printInfo("初始化：measureHeight="+measureHeight);
//		printInfo("初始化：contextMaxSize="+contextMaxSize);
//		printInfo("初始化：contextMinSize="+contextMinSize);
...
//		}
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            for (LockCircle lockCircle : cycleList) {
                if (lockCircle.isPointIn(eventX, eventY)) {
                    lockCircle.setTouch(true);
                    if (!mPasswordList.contains(lockCircle.getNum())) {
                        mPasswordList.add(lockCircle.getNum());
        ...
        case MotionEvent.ACTION_UP:
            ...
            if (avertUnawareTouch && len <= 1) {
                reset();
            } else if (len < limitNum) {
                callError(ERROR_SHORT);
...
//		canvas.restore();
    ...
    public static class LockCircle {
        ...
        /**
         * 判读传入位置是否在圆心内部。
         * 点击后会形成一个矩形，它的对角线其实就是点击位置到圆心位置的距离，
         * 也就是直角三角形的对边长，这个距离如果<=半径，那就是在圆内，否则在圆外。
         *
         * @param x 当前触摸的X轴
         * @param y 当前触摸的Y轴
         * @return 是否在圆心内部
    ...
    public boolean isAvertUnawareTouch() {
        return avertUnawareTouch;
    }

    /**
     * 设置是否开启防误触碰功能（默认是开启的）
     *
     * @param avertUnawareTouch
     */
    public void setAvertUnawareTouch(boolean avertUnawareTouch) {
        this.avertUnawareTouch = avertUnawareTouch;
    }
